...
@boundscheck(False)
@wraparound(False)
cdef list[str] get_color_codes(cnp.uint8_t[:, :] data, bint enhance_color = True):
    '''
    获取颜色映射代码列表
    ---
    ...
    cdef:
        ...
        cnp.ndarray[cnp.uint8_t, ndim=2] _labs = cv.cvtColor(rgbs, cv.COLOR_RGB2LAB).reshape(-1, 3)
        cnp.ndarray[cnp.uint8_t, ndim=3] labs
        # 将 rgb 颜色列表转换为 lab 颜色列表，shape=(M, 3)
        cnp.ndarray[cnp.uint8_t, ndim=2] _lab_colors = cv.cvtColor(colors, cv.COLOR_RGB2LAB).reshape(-1, 3)
        cnp.ndarray[cnp.uint8_t, ndim=3] lab_colors
    ...
    if M < N:
        ...
        lab_colors = _lab_colors[:, np.newaxis, :]
        distances = np.asarray([deltaE_ciede2000(_labs, lab_color, channel_axis=-1) for lab_color in lab_colors]).T
    else:
        # 计算所有颜色之间的距离，shape=(N, M)
        labs = _labs[:, np.newaxis, :]
        distances = np.asarray([deltaE_ciede2000(_lab_colors, lab, channel_axis=-1) for lab in labs])
    # 找到最小距离的索引，shape=(N, )
